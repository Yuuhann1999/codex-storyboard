...
> **Storyboard Workbench 开发分支说明**：本仓库正在从 Codex Storyboard 深度扩展为通用分镜工作台。第一阶段已建立独立插件身份、用户自选项目目录、项目数据结构和本地工作区 API。分镜创作、审批与交付能力会分阶段接入；LibTV 视频生成保持为外部独立技能，不在本工作台内执行。
...
## Storyboard Workbench 开发

当前开发入口统一使用插件内置应用，避免仓库根目录与插件目录维护两份服务代码：
...
`npm start` 默认监听 `http://127.0.0.1:43219`，开发配置写入仓库的 `data/`。新工作区通过 `POST /api/workspaces` 创建，完整项目内容保存在用户指定目录的 `.storyboard/project.json`；全局配置目录只记录最近项目的路径、标题和打开时间。

第一阶段只提供工作区基础设施。现有旧版项目和素材接口暂时保留，以便后续逐步迁移 UI 与 MCP 工具。
